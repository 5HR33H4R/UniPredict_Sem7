...
def result():
    ...
    if (input_rank == ''):
        m2r = pickle.load(open('marks_to_rank.pickle', 'rb'))
        r2r = pickle.load(open('rank_to_round.pickle', 'rb'))

        # with open('marks_to_rank.pickle', 'rb') as f:
        #    m2r = pickle.load(f)
        # with open('rank_to_round.pickle', 'rb') as f:
        #    r2r = pickle.load(f)

        temp = m2r.predict([[input_marks]])
        input_rank = round(float(temp[0][0]))
        temp = r2r.predict([[input_rank]])
        input_round = round(float(temp[0][0]))
        print('Predicted Rank: ', input_rank)
        print('Predicted Round: ', input_round)
    ...
    return render_template("index.html", percent=random.randint(55, 75), dept=list_Branch_Names, College_name=list_College_Names, College_code=list_College_Codes, dept_code=list_Branch_Codes, com=list_Community, predRank=display_pred_rank, predRound=display_pred_round)
